...
> - **回填进度**：本表当前**覆盖全部 175 条条目（不分等级，逐条由门禁第 34 项强制——REFERENCES 键/arXiv 号 ⊆ LOG 首列，欠账复发即 FAIL）**。历史演进：G 类 51 条先行（豆包审查建议 4）→ v2.26.1 补登至 134 条（⚠️ v2.19.0–v2.26.0 期间新增的 10 条 B 级曾漏登记——门禁第 34 项当时只强制 G 键覆盖，构成盲区）→ v2.44.0 清欠至全量 175 条（A:9 / B:104 / C:10 / D:1 / G:51）。文末各批次段落为分批历史记录，时点计数以当时为准。
> - **核验时效（staleness）政策（v2.44.2 新增，外部审查 A3 回应）**：核验是对「当时可达一手来源」的记录，不等于永久有效——引用可能被撤回、替代或后续证伪。时效规则：① **B / C 级预印本类条目**入库后**每 6 个月**须在最近一次文献巡逻中复核（撤回 / 大版本替换 / 证伪 → 走证据修正流程，删除需举证）；② **G 类一手监管 / 执法条目**在法源修订或事件续报时按需重验，登记新核验日期；③ **每周一文献巡逻的固定输入 = 本表「核验日期」最早的前 20 条**。本政策为流程条款（暂无脚本化门禁），执行载体见 CONTRIBUTING「3.5 流程纪律」第 ⑤ 条。
...
## v2.43.0 新增 A 类条目核验登记（1 条，C12 标定批）
...
| AOE-CALIB-006 | 量规 v2.1 复验（自复现） | v2.46.0 | 2026-09-16 | 浏览器实测（脚本复现）+ 零依赖 --selftest 5/5 | 本仓库 `calibrate-c12-v21-reverify.mts`（三数据集确定性重投影）+ `CALIBRATION-REPORT-C12-V21.md` + 留痕 `CALIBRATION-C12-V21-REVERIFY-1789567706991.json` + 预注册 `EXPERIMENT-DESIGN-AOE-CALIB-006.md`（c24491a 先于执行入库）；数据集与方法论同产（张金瑞，单作者）；⚠️ 预注册空集语义歧义按偏差双读法登记（两跑留痕均保留） |
...
## v2.44.0 历史欠账清欠补登记（35 条：v2.27.0–v2.41.0 期间新增）

> ⚠️ **计数修正**：v2.41.1 登记的欠账估算为「36 条（A +5 / B +30 / C +1）」，本次逐条清点实为 **35 条**（A 5 / B 29 / C 1）——差 1 条为 AOE-CALIB-004，已于 v2.43.0 入库时即登记，不属欠账。核验方式与一手来源取自各条目入库时 CHANGELOG 所载的实际核验记录；5 条 A 级自复现实验沿用「浏览器实测（脚本复现）」口径。

| 条目键 | 类别 | 入库版本 | 核验日期 | 核验方式 | 一手来源（官方渠道） |
|---|---|---|---|---|---|
| AOE-ABSTAIN-001 | 弃权配对代理（自复现） | v2.33.0 | 2026-09-14（补登记） | 浏览器实测（脚本复现） | 本仓库 `reference/runtimes/l2-runtime-oversight/` 对应脚本 + 报告 + 预注册；数据集与方法论同产（张金瑞，单作者） |
| AOE-ABSTAIN-002 | 弃权隐式条件（自复现） | v2.34.0 | 2026-09-14（补登记） | 浏览器实测（脚本复现） | 本仓库 `reference/runtimes/l2-runtime-oversight/` 对应脚本 + 报告 + 预注册；数据集与方法论同产（张金瑞，单作者） |
| AOE-CALIB-003 | 指挥集中度 ROC 标定（自复现） | v2.37.0 | 2026-09-14（补登记） | 浏览器实测（脚本复现） | 本仓库 `reference/runtimes/l2-runtime-oversight/` 对应脚本 + 报告 + 预注册；数据集与方法论同产（张金瑞，单作者） |
| AOE-DETECT-001 | 配置审查检出率（自复现） | v2.32.0 | 2026-09-14（补登记） | 浏览器实测（脚本复现） | 本仓库 `reference/runtimes/l2-runtime-oversight/` 对应脚本 + 报告 + 预注册；数据集与方法论同产（张金瑞，单作者） |
| AOE-SWARM-001 | 蜂群信号检出率（自复现） | v2.35.0 | 2026-09-14（补登记） | 浏览器实测（脚本复现） | 本仓库 `reference/runtimes/l2-runtime-oversight/` 对应脚本 + 报告 + 预注册；数据集与方法论同产（张金瑞，单作者） |
| AHE | 可观测性驱动可证伪契约 | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2604.25850（arXiv 摘要页一手核验；本行为补登记） |
| Agent-Zero-Memory | 引文锁记忆架构 | v2.30.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.29606（arXiv 摘要页一手核验；本行为补登记） |
| AgentWorm | 跨实例自传播蠕虫 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2603.15727（arXiv 摘要页一手核验；本行为补登记） |
| Aspire | 模糊目标过程跑通≠能力提升 | v2.31.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.31111（arXiv 摘要页一手核验；本行为补登记） |
| CPV-Gate | 通信声明溯源验证门控 | v2.27.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.06830（arXiv 摘要页一手核验；本行为补登记） |
| Evo-Harness | 上下文到 harness 技能编译 | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.15071（arXiv 摘要页一手核验；本行为补登记） |
| EvolveNet | 协作式 harness 演化 | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.04968（arXiv 摘要页一手核验；本行为补登记） |
| FakeLab | 逐实例监控分布式攻击失效 | v2.27.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2607.07368（arXiv 摘要页一手核验；本行为补登记） |
| GhostWriter | 记忆治理缺失根因 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2607.06595（arXiv 摘要页一手核验；本行为补登记） |
| HANSARD | 归因洗白参考架构（C） | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.22512（arXiv 摘要页一手核验；本行为补登记） |
| HSI | 任务特定 harness 骨干能力界 | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.08466（arXiv 摘要页一手核验；本行为补登记） |
| HarnessAudit | Harness 安全审计 | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2605.14271（arXiv 摘要页一手核验；本行为补登记） |
| HarnessDev | Harness 演化否定性边界 | v2.31.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2609.01437（arXiv 摘要页一手核验；本行为补登记） |
| HarnessSafe | 持久化载体跨系统威胁 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.06984（arXiv 摘要页一手核验；本行为补登记） |
| Institutional-Red-Teaming | 部署规则塑造多 Agent 安全 | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2607.07695（arXiv 摘要页一手核验；本行为补登记） |
| MAFIA | 探测式记忆投毒 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.03844（arXiv 摘要页一手核验；本行为补登记） |
| MAStrike | 监察点优先级 Shapley 归因 | v2.27.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2606.12918（arXiv 摘要页一手核验；本行为补登记） |
| MemMorph | 长期记忆工具劫持 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2605.26154（arXiv 摘要页一手核验；本行为补登记） |
| Proactive-Memory-Agent | 记忆作为主动干预 | v2.30.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2607.08716（arXiv 摘要页一手核验；本行为补登记） |
| RA-6 | 部署前本体论验证 | v2.29.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2606.04037（arXiv 摘要页一手核验；本行为补登记） |
| Router-Mem | 证据充分性路由 | v2.30.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.01285（arXiv 摘要页一手核验；本行为补登记） |
| S3Gym | 自判不能替代独立固化 | v2.31.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.31100（arXiv 摘要页一手核验；本行为补登记） |
| SkillFortify | Skill 供应链安全分析 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2603.00195（arXiv 摘要页一手核验；本行为补登记） |
| State-Contamination | 记忆漂洗与消毒时机 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2605.16746（arXiv 摘要页一手核验；本行为补登记） |
| VISTA | 上下文预算自身感知 | v2.30.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2606.30005（arXiv 摘要页一手核验；本行为补登记） |
| VLA-latent | 隐状态通信通道 | v2.27.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.19161（arXiv 摘要页一手核验；本行为补登记） |
| WMT | 检索侧防御有限缓解 | v2.28.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2608.20631（arXiv 摘要页一手核验；本行为补登记） |
| emergent-social-intelligence | 多 Agent 失效统计常态 | v2.27.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2603.27771（arXiv 摘要页一手核验；本行为补登记） |
| emergent-whistleblowing | 内生吹哨涌现实例 | v2.27.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2609.04170（arXiv 摘要页一手核验；本行为补登记） |
| prompt-collusion | Prompt 优化层涌现共谋 | v2.27.0 | 2026-09-14（补登记） | arXiv 摘要页一手核验（入库时执行） | arXiv:2604.17774（arXiv 摘要页一手核验；本行为补登记） |
| DP-PREVIEW-001 | DeepPattern 托管审查实测（权威前提注入对照实验） | v2.48.2 | 2026-09-18（入库时执行） | 参与者实测留档核验（审计记录 + 测试仓库 git 历史逐轮回溯；本地私有未公开，C 级） | 本地留档：测试仓库 dp-audit-repo（git 历史含全部审查工件）+ 《DeepPattern外测_审计记录_T1-T5.md》+ 反馈报告 v2（无公开来源，C 级依据） |
| cn-fw30-2026 | 中国·标准 | v2.47.0 | 2026-09-18 | 官方原文核验 | 官方 PDF 原文逐字核验（用户提供文件 136 页，封面署名与发布日期核验；发布事实与 TC260 官网 www.tc260.org.cn、人民网 2026-09-16 第 06 版、中国网、中国工信新闻网四路交叉一致；关键条款逐字比对见 spec/06 威胁面对照节） |

✅ **v2.44.0 历史欠账 35 条清欠完成。自此 VERIFICATION-LOG 覆盖 REFERENCES 全部 175 条条目；门禁第 34 项已同步扩展为「不分等级、全量键覆盖」，欠账复发路径被脚本堵死。**

| cn-renmin-air-2026 | 人大《人工智能安全治理研究报告（2026 年）》（C） | v2.51.0 | 2026-09-20（入库时执行） | 报告自述要素登记（封面 / 目录自述；全文提取留存于编制方工作区；未做逐字核验，C 级） | 91 页白皮书，机构与标题取自报告封面自述。登记即带引用红线：参考文献逐章重新编号——逐章对照表建立前不得引用任何编号转引；未溯源数字禁引；二级来源回溯挂账（Khalaf 推理期奖励投机 / Arike 目标漂移 / Bengio International AI Safety Report 2026 / ICLR 2025 浅层对齐 / CVE-2026-5757、CVE-2026-33626 可经 NVD 核验）。当前无 spec 直接引用 |

| deepseek-dsec-2026 | 学术预印本（B） | v2.51.2 | 2026-09-25（入库时执行） | arXiv 摘要页 + HTML 全文逐条核验，七项关键内容全部确认（§6.4 六类不当行为案例 / §6.5「无单一机制」原文 / §6.1 构建运行分账号并清理残留答案 / §3.3+§7 Edge 二次准入 / §4.3 负载形态统计 / §6.2–6.3 解耦与挂起恢复）；提交信息（131+ 作者、梁文锋列末位、ATC 2026 OSC 首轮已过）取自 arXiv 页面字段；媒体交叉（澎湃 / 联合早报 / 网易）仅用于发布事实确认，不作引用锚点 | arxiv.org/abs/2609.22978（HTML 全文 arxiv.org/html/2609.22978v1）。⚠️ 机构自报生产数据不可独立复现（COI 已挂）、治理对象为训练沙盒（域差已挂）、未经同行评审（B 级取方向性结论） |

| arXiv:2207.05221 | 学术预印本（B） | v2.51.4 | 2026-09-25（入库时执行） | arXiv 摘要页一手核验（WebFetch 逐字段：标题 / 36 位作者 / v1 2022-07-11、v4 2022-11-21 / 摘要含 P(IK) 跨任务校准挣扎的不利限定） | arXiv:2207.05221（Language Models (Mostly) Know What They Know，Anthropic；正面结论与不利限定均经摘要原文确认） |

| arXiv:2512.16030 | 学术预印本（B） | v2.51.4 | 2026-09-25（入库时执行） | arXiv 摘要页一手核验（WebFetch 逐字段：标题 / 单作者 Lukas Nel / v1 2025-12-17 / ECE 0.120 与 0.395 / Brier Skill Score 结论） | arXiv:2512.16030（KalshiBench，300 道 CFTC 监管预测市场题；系统性过自信与「仅一模型 BSS 为正」均经摘要原文确认） |

| Steyvers | 期刊论文（B） | v2.51.4 | 2026-09-25（入库时执行） | CrossRef 注册元数据逐字段核验（nature.com 反爬拦截，改走官方 DOI API；检索索引摘要交叉印证） | DOI 10.1038/s42256-024-00976-7（Nature Machine Intelligence 7(2):221–231，2025-01-21 在线；8 位作者 / 收稿接受日期 / 无竞争利益声明均经注册记录确认） |

| Farquhar | 期刊论文（B） | v2.51.4 | 2026-09-25（入库时执行） | CrossRef 注册元数据逐字段核验（nature.com 反爬拦截，改走官方 DOI API；检索索引摘要交叉印证） | DOI 10.1038/s41586-024-07421-0（Nature 630(8017):625–630，2024-06-19 在线；4 位作者 / 收稿接受日期 / 利益披露（两位作者现职 GDM、OpenAI）均经注册记录确认） |