...
这是 36 条未解问题中的 #15（见 `spec/13-boundaries.md`），也是社区共建的优先方向。敢于公开承认「狼群我们还没搞定」，比声称「我们能搞定一切」更能建立专家社区的信任。
...
2. **可核查文献**（179 条来源，其中 B 级 104 条，全部含 DOI/arXiv 号；G 级 52 条治理 / 政策 / 执法一手实证，覆盖中、美、英、加、澳、欧盟、新加坡、韩国、日本九法域及 ISO、OWASP 两个国际标准组织）：
...
当前分布：A:11（Sandlock，arXiv:2605.26298，实验复现 / AOE-CALIB-001，影子比阈值自复现标定 / AOE-CALIB-002，周期检测方差阈值标定 / AOE-DETECT-001，配置审查检出率覆盖枚举 / AOE-ABSTAIN-001，弃权率配对代理测量 / AOE-ABSTAIN-002，弃权率隐式条件实验 / AOE-SWARM-001，蜂群检测信号检出率复现 / AOE-CALIB-003，指挥集中度阈值 ROC 标定 / AOE-CALIB-004，C12 信用分量规真实任务标定 / AOE-CALIB-005，C12 量规 v2 重投影标定 / AOE-CALIB-006，C12 量规 v2.1 标定复验）/ B:104 / C:11 / D:1 / G:52。**「证据不足」是可审阅的状态，不是被隐藏的缺陷。**

> **结构性偏置声明**：当前 B 级约占 58%（104/179），A 级 11 条（Sandlock 实验复现 + AOE-CALIB-001 影子比阈值标定 + AOE-CALIB-002 周期检测方差阈值标定 + AOE-DETECT-001 配置审查检出率覆盖枚举 + AOE-ABSTAIN-001 弃权率配对代理测量 + AOE-ABSTAIN-002 弃权率隐式条件实验 + AOE-SWARM-001 蜂群检测信号检出率复现 + AOE-CALIB-003 指挥集中度阈值 ROC 标定 + AOE-CALIB-004 C12 信用分量规真实任务标定 + AOE-CALIB-005 C12 量规 v2 重投影标定 + AOE-CALIB-006 C12 量规 v2.1 标定复验）；G 级 52 条约占 29%（52/178），覆盖 L0–L4 + E 六个落点与九个法域；另有 1 条 C 级外部产品实测（DP-PREVIEW-001，DeepPattern 托管审查对照实验，本地留档未公开）。这反映本框架当前形态是"规范类项目 + 真实治理实证锚点 + 自复现标定实验"的混合结构——学术引用多于一手可复现实验，但治理与执法实证已实现跨法域广泛覆盖，且本体系自身的运行数据已开始填补工程参数标定缺口。**不应被解读为实证充分**。把 A 级占比提升上去的路径已在 `spec/13-boundaries.md` 的开放问题中列出（尤其是与实验性验证相关的条目），也是本框架从"规范"走向"有实证支撑的可引用规范"的关键一步。
...
- 当前版本：v2.48.2（见 [`VERSION`](VERSION)；本行由门禁第 38 项与 VERSION 联动校验）
- 维护状态：**不声称建成，只声称维护状态**——开放作弊集合只能持续对抗
- **诚实边界**：36 条未解问题显式登记（见 `spec/13-boundaries.md`）——这是本框架的诚实边界，也是研究者的选题清单；含全体系最大的工程缺口「去中心化蜂群反制」
...
3. 读 [`spec/13-boundaries.md`](spec/13-boundaries.md) 的 36 条未解问题，挑一条开 Issue 讨论。
...
4. **研究者**：从 `REFERENCES.md` 出发复现证据链，或从 `spec/13-boundaries.md` 的 36 条未解问题选题；
