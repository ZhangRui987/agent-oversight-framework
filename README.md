...
这是 15 条未解问题中的 #15（见 `spec/13-boundaries.md`），也是社区共建的优先方向。敢于公开承认「狼群我们还没搞定」，比声称「我们能搞定一切」更能建立专家社区的信任。
...
- 当前版本：v1.2.0（见 [`VERSION`](VERSION)）
- 维护状态：**不声称建成，只声称维护状态**——开放作弊集合只能持续对抗
- **诚实边界**：15 条未解问题显式登记（见 `spec/13-boundaries.md`）——这是本框架的诚实边界，也是研究者的选题清单；含全体系最大的工程缺口「去中心化蜂群反制」
...
3. 读 [`spec/13-boundaries.md`](spec/13-boundaries.md) 的 15 条未解问题，挑一条开 Issue 讨论。
...
4. **研究者**：从 `REFERENCES.md` 出发复现证据链，或从 `spec/13-boundaries.md` 的 15 条未解问题选题；
