...
这是 25 条未解问题中的 #15（见 `spec/13-boundaries.md`），也是社区共建的优先方向。敢于公开承认「狼群我们还没搞定」，比声称「我们能搞定一切」更能建立专家社区的信任。
...
2. **可核查文献**（55 条来源，其中 B 级 45 条，全部含 DOI/arXiv 号）：
...
当前分布：A:1（Sandlock，arXiv:2605.26298，实验复现）/ B:45 / C:8 / D:1。**「证据不足」是可审阅的状态，不是被隐藏的缺陷。**
...
- 当前版本：v1.6.0（见 [`VERSION`](VERSION)）
- 维护状态：**不声称建成，只声称维护状态**——开放作弊集合只能持续对抗
- **诚实边界**：25 条未解问题显式登记（见 `spec/13-boundaries.md`）——这是本框架的诚实边界，也是研究者的选题清单；含全体系最大的工程缺口「去中心化蜂群反制」
...
3. 读 [`spec/13-boundaries.md`](spec/13-boundaries.md) 的 25 条未解问题，挑一条开 Issue 讨论。
...
4. **研究者**：从 `REFERENCES.md` 出发复现证据链，或从 `spec/13-boundaries.md` 的 25 条未解问题选题；
