...
2. **可核查文献**（102 条来源，其中 B 级 58 条，全部含 DOI/arXiv 号；G 级 33 条治理 / 政策 / 执法一手实证，覆盖中、美、英、加、欧盟、新加坡、韩国、日本、ISO、OWASP 七法域）：
...
当前分布：A:1（Sandlock，arXiv:2605.26298，实验复现）/ B:58 / C:9 / D:1 / G:33。**「证据不足」是可审阅的状态，不是被隐藏的缺陷。**

> **结构性偏置声明**：当前 B 级约占 57%（58/102），A 级仅 1 条；G 级 33 条占 32%（33/102），覆盖 L0–L4 六个落点与七个法域。这反映本框架当前形态是"规范类项目 + 真实治理实证锚点"的混合结构——学术引用多于一手可复现实验，但治理与执法实证已实现跨法域广泛覆盖。**不应被解读为实证充分**。把 A 级占比提升上去的路径已在 `spec/13-boundaries.md` 的开放问题中列出（尤其是与实验性验证相关的条目），也是本框架从"规范"走向"有实证支撑的可引用规范"的关键一步。
