...
- Current version: v1.3.0 (see [`VERSION`](VERSION))
- Maintenance status: **does not claim "built"; claims only "maintained"** — an open cheating set can only be countered continuously
- **Honest boundary**: 20 open problems explicitly registered (see `spec/13-boundaries.md`) — this framework's honest boundary and a problem list for researchers; includes the system's largest engineering gap, "decentralized-swarm countermeasures"
...
3. Read the 20 open problems in [`spec/13-boundaries.md`](spec/13-boundaries.md), pick one, and open an issue to discuss.
...
4. **Researchers**: reproduce the evidence chain from `REFERENCES.md`, or select a topic from the 20 open problems in `spec/13-boundaries.md`;
