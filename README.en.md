...
2. **Verifiable literature** (61 sources, of which 50 are grade B, all with DOI/arXiv identifiers):
...
Current distribution: A:1 (Sandlock, arXiv:2605.26298, experimental reproduction) / B:50 / C:9 / D:1. **"Insufficient evidence" is a reviewable state, not a hidden defect.**

> **Structural-bias disclosure**: grade B currently accounts for about 82% (50/61), with only 1 grade-A source. This reflects the natural shape of a specification-first project at this stage — policy documents, authoritative institutional reports, and peer preprints are cited more often than independently reproducible experiments. **This distribution should not be read as empirical sufficiency.** The path to raising the grade-A share is already listed among the open problems in `spec/13-boundaries.md` (particularly the entries related to experimental validation), and is the key step for this framework to move from "a specification" to "a citable specification with empirical backing."
...
- Current version: v2.3.0 (see [`VERSION`](VERSION))
...
| `REFERENCES.md` | 61 graded evidence sources (single source of truth) |
