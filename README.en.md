...
[![Version](https://img.shields.io/badge/version-2.51.6-blue)](VERSION)
[![License: CC BY-SA 4.0](https://img.shields.io/badge/License-CC_BY--SA_4.0-lightgrey)](LICENSE)
[![Evidence: B (109/185)](https://img.shields.io/badge/evidence-B_109%2F185-yellow)](spec/11-traceability.md)
...
This is open problem **#15** of 37 (see `spec/13-boundaries.md`), and a priority direction for community co-development. Publicly admitting "we haven't solved wolf-packs yet" builds more trust with the expert community than claiming "we can handle everything."

Also explicitly registered: **10 of the 11 grade-A evidence items are self-reproduction experiments by this framework, and the count of independent third-party replications is currently zero** — grade A means "first-hand, verifiable, reproducible", not "independently verified". Every experiment's script, preregistration, and raw output are in the repository; the per-item replication conditions and submission channel are in `spec/13-boundaries.md` § "Independent-verification status of grade-A evidence and the third-party replication path" (issue template `independent-replication`).
...
2. **Verifiable literature** (185 sources, of which 109 are grade B — all with DOI/arXiv identifiers — and 52 are grade G governance/policy/enforcement primary evidence spanning CN/US/UK/CA/AU/EU/SG/KR/JP + ISO + OWASP):
   ...
   - AI Safety Governance Framework 3.0 (TC260, 2026-09, 【键: cn-fw30-2026】) — the national agentic-AI risk-management framework (Annex 2: 9 categories / 33 risk items + 7 classes of safeguards). This framework interfaces with it as the **verification layer**: the standard says what shall be done, this framework provides how to prove it was done;
...
- Current version: v2.51.6 (see [`VERSION`](VERSION); this line is checked against VERSION by consistency check #38)
- Maintenance status: **does not claim "built"; claims only "maintained"** — an open cheating set can only be countered continuously
- **Honest boundary**: 39 open problems explicitly registered (see `spec/13-boundaries.md`) — this framework's honest boundary and a problem list for researchers; includes the system's largest engineering gap, "decentralized-swarm countermeasures"
...
3. Read the 39 open problems in [`spec/13-boundaries.md`](spec/13-boundaries.md), pick one, and open an issue to discuss.
...
4. **Researchers**: reproduce the evidence chain from `REFERENCES.md`, or select a topic from the 39 open problems in `spec/13-boundaries.md`;
...
| `REFERENCES.md` | 185 graded evidence sources (single source of truth) |
...
> APA: Zhang, R. (2026). *AI Execution and Oversight Framework* (v2.51.6). https://github.com/ZhangRui987/agent-oversight-framework
