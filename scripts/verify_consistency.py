...
校验项（共 35 项，按运行顺序）：
  ...
  6. REFERENCES 实测 C:9
  7. REFERENCES 实测 D:1
  8. REFERENCES 实测 G:48
  9. REFERENCES 合计 119 条
  10. README 声明「119 条来源，其中 B 级 58 条」
  11. README 分布「A:3 / B:58 / C:9 / D:1」（含 A 级来源标注）
  12. CHANGELOG 声明参考文献条数（取最近一轮变更后的值）
  13. REFERENCES 注脚「58 条 B 级、9 条 C 级、1 条 D 级、48 条 G 级（合计 119 条来源）」
  14. README 双语全文「证据总数」一致（所有出现的总数 == 119）
...
check("REFERENCES 实测 G:48", g_count == 48, f"实测 G={g_count}")
total = a + b + c + d + g_count
check("REFERENCES 合计 119 条", total == 119, f"实测合计={total}")

readme = read(os.path.join(ROOT, "README.md"))
chg = read(os.path.join(ROOT, "CHANGELOG.md"))
check("README 声明「119 条来源，其中 B 级 58 条」",
      "119 条来源，其中 B 级 58 条" in readme)
...
check("CHANGELOG 声明「新增 1 条 G 级」（v2.11.0）",
      "新增 1 条 G 级" in chg)
check("REFERENCES 注脚「58 条 B 级、9 条 C 级、1 条 D 级、48 条 G 级（合计 119 条来源）」",
      "58 条 B 级、9 条 C 级、1 条 D 级、48 条 G 级（合计 119 条来源）" in refs)
