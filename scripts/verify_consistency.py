...
校验项（共 39 项 = 39 个 check( 调用，按运行顺序；v2.26.1 起本清单项数与实际调用数一致，由第 39 项自校验）：
  ...
  9. REFERENCES 合计 170 条
  10. README 声明「170 条来源，其中 B 级 100 条」
  11. README 分布「A:8 / B:100 / C:10 / D:1」（含 A 级来源标注）
  12. CHANGELOG 声明「新增 1 条 G 级」（v2.11.0，历史锚点防回退）
  13. REFERENCES 注脚「8 条 A 级、100 条 B 级、10 条 C 级、1 条 D 级、51 条 G 级（合计 170 条来源）」
  14. README 双语全文「证据总数」一致（所有出现的总数 == 170）
...
check("REFERENCES 合计 170 条", total == 170, f"实测合计={total}")

readme = read(os.path.join(ROOT, "README.md"))
chg = read(os.path.join(ROOT, "CHANGELOG.md"))
check("README 声明「170 条来源，其中 B 级 100 条」",
      "170 条来源，其中 B 级 100 条" in readme)
check("README 分布「A:8 / B:100 / C:10 / D:1」（含 A 级来源标注）",
      "A:8" in readme and "B:100 / C:10 / D:1" in readme)
# CHANGELOG 取最近版本声明——门禁只校验字符串存在，数值一致性由第 14-15 项覆盖
# v2.11.0 条目含「新增 1 条 G 级」字样
check("CHANGELOG 声明「新增 1 条 G 级」（v2.11.0）",
      "新增 1 条 G 级" in chg)
check("REFERENCES 注脚「8 条 A 级、100 条 B 级、10 条 C 级、1 条 D 级、51 条 G 级（合计 170 条来源）」",
      "8 条 A 级、100 条 B 级、10 条 C 级、1 条 D 级、51 条 G 级（合计 170 条来源）" in refs)
