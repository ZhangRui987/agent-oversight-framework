...
校验项（共 41 项 = 41 个 check( 调用，按运行顺序；v2.26.1 起本清单项数与实际调用数一致，由第 39 项自校验）：
  ...
  6. REFERENCES 实测 C:11
  7. REFERENCES 实测 D:1
  8. REFERENCES 实测 G:52
  9. REFERENCES 合计 179 条
  10. README 声明「179 条来源，其中 B 级 104 条」
  11. README 分布「A:11 / B:104 / C:11 / D:1」（含 A 级来源标注）
  12. CHANGELOG 声明「新增 1 条 G 级」（v2.11.0，历史锚点防回退）
  13. REFERENCES 注脚「11 条 A 级、104 条 B 级、11 条 C 级、1 条 D 级、52 条 G 级（合计 179 条来源）」
...
check("REFERENCES 实测 C:11", c == 11, f"实测 C={c}")
check("REFERENCES 实测 D:1", d == 1, f"实测 D={d}")
check("REFERENCES 实测 G:52", g_count == 52, f"实测 G={g_count}")
total = a + b + c + d + g_count
check("REFERENCES 合计 179 条", total == 179, f"实测合计={total}")

readme = read(os.path.join(ROOT, "README.md"))
chg = read(os.path.join(ROOT, "CHANGELOG.md"))
check("README 声明「178 条来源，其中 B 级 104 条」",
      "179 条来源，其中 B 级 104 条" in readme)
check("README 分布「A:11 / B:104 / C:10 / D:1」（含 A 级来源标注）",
      "A:11" in readme and "B:104 / C:11 / D:1" in readme)
# CHANGELOG 取最近版本声明——门禁只校验字符串存在，数值一致性由第 14-15 项覆盖
# v2.11.0 条目含「新增 1 条 G 级」字样
check("CHANGELOG 声明「新增 1 条 G 级」（v2.11.0）",
      "新增 1 条 G 级" in chg)
check("REFERENCES 注脚「11 条 A 级、104 条 B 级、11 条 C 级、1 条 D 级、52 条 G 级（合计 179 条来源）」",
      "11 条 A 级、104 条 B 级、11 条 C 级、1 条 D 级、52 条 G 级（合计 179 条来源）" in refs)
