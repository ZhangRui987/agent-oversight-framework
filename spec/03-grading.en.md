...
### Model-level ↔ task-level: orthogonally complementary mapping of G×S with external "capability threshold × safety measure" frameworks (new in v2.21.0)

G×S is a **task-level** grading (given a task, determine its risk and scale grade), while the three major frontier-lab frameworks — Anthropic RSP (Responsible Scaling Policy), OpenAI Preparedness, and DeepMind FSF (Frontier Safety Framework) — are **model-level** grading (given a model, determine corresponding safety measures based on its capability thresholds). The two measure different objects but are structurally isomorphic: both are checkpoint-style mappings of "input grade → output measures." Placing them together is neither substitution nor stacking, but **orthogonal complementarity** — the model-level framework answers "what is this model allowed to do," while the task-level framework answers "what kind of oversight does this task require."

The three frameworks share a common structure: they define several **capability thresholds** (e.g., CBRN capability, autonomy capability, cyber-offense capability), each bound to a **safety-measure checkpoint** (e.g., red-team intensity, monitoring deployment density, release-approval level). This framework does not evaluate the correctness of each lab's threshold settings — those are internal policies — and only engages with their structure.

The mapping between model-level (external frameworks) and task-level (G×S) is as follows:

| External framework's capability threshold | Corresponding model autonomy tier | Corresponding G-grade range (mapping as heuristic, not hard binding) | Relationship |
| --- | --- | --- | --- |
| Below threshold (model under all defined thresholds) | Low autonomy | G0–G1 | Model capability does not exempt task-side risk assessment |
| First threshold met (e.g., autonomy preliminarily demonstrated) | Medium autonomy | G2 | Model-level measures and task-level oversight trigger in parallel |
| High threshold met (e.g., capable of chaining multi-step exploit chains) | High autonomy | G3–G4 | Task-level oversight intensity must match the model-level danger rating |

Two key constraints follow directly from this mapping:

(1) **Task horizon is not exempted by model tier** — this is an existing provision of this framework (see "task horizon is not exempted by model capability" below); the model-level mapping provides its external counterpart: even if a model is classified by its developer as "low-threshold," a task with large horizon, high externality, and high irreversibility **still** must trigger its own G-grade oversight intensity based on its own risk profile. Conversely, even if the model is "high-threshold," a G0 task should not be escalated to G3 oversight because of model tier — **oversight intensity follows the task, not the model**.

(2) **The dual-layer matrix produces a combined declaration, not a substitution choice.** When admitting a task, the deployer should declare simultaneously "which capability tier the model in use occupies" and "which G×S grade the task falls in," and check whether the two are compatible — for example, "high-threshold model + G4 task" must incorporate the model-level safety measures (such as more intensive red-teaming, finer-grained monitoring) into the task-level oversight combination, rather than running each independently.

The supply side of external risk catalogs has also produced mature references. The MIT AI Risk Repository ([key: MIT-AI-Risk-Repository] Peter Slattery, Alexander K. Saeri, Emily A. C. Grundy et al., 10 authors; arXiv:2408.12622, v1 2024-08-14 / v3 2026-05-05; published in *Patterns* (Cell Press) 2026, DOI 10.1016/j.patter.2026.101517; CC BY 4.0 open access) systematically analyzed all major AI-risk frameworks as of the paper version — **74 frameworks, 1,725 deduplicated distinct risks** — and produced two classification systems: a causal taxonomy (entity: human/AI × intent × timepoint) and a domain taxonomy (7 domains, 24 subdomains). One key finding: **human decisions cause nearly as many AI risks (38%) as AI systems themselves (42%)** — this is the external corroboration of the same proposition that makes this framework place L1 (task-definition QA) and L0 (training alignment) alongside each other as governance foci, rather than governing only L2 (runtime oversight).

That risk repository can serve as an **external supply source of risk-catalog evidence** for this framework's G axis (task-risk dimension): the G-axis criteria (impact scope / externality / irreversibility) can interface with its causal taxonomy (entity dimension maps to "who is the source of impact," intent dimension maps to "intentional or unintentional," timepoint dimension maps to "before/during/after deployment"), continuously replenishing the governance-evidence anchors needed by this framework's REFERENCES.

⚠️ Evidence grade B. Four limitations must be retained: ① the specific provisions of the three lab frameworks (RSP / Preparedness / FSF) have **not been individually verified against their primary texts** by this framework — citations are based on publicly released framework summaries and news reports, and **must not** be used to claim that "a given framework prescribes a specific threshold"; the three frameworks are their institutions' internal policies, and this framework does not evaluate their correctness, only engages with their structure. ② The MIT AI Risk Repository's "1,725 risks / 74 frameworks" figure is from its **v1 paper version (2024-08)**; subsequent versions (v3 2026-05) may have expanded it, so **the version must be cited**; and its classification system is the product of a **meta-review** (synthesizing existing frameworks), not primary measurement. ③ **The lab frameworks are internal institutional policies; AOF does not evaluate their correctness, only engages with their structure** — this section must not be read as "this framework endorses any particular lab's threshold settings." ④ The "corresponding G-grade range" in the table above is a **heuristic of structural mapping**, not a hard binding — the same model can be used for tasks of different G grades; the table gives only "typical compatible ranges."
...
**Task horizon is not waived by model capability (added this round).** Measured frontier-model performance on long-horizon reasoning shows that "the model in use is strong enough" is not an acceptable grounds for downgrading: LongCoT (arXiv:2604.14140, 2026-04-15, DOI 10.48550/arXiv.2604.14140) measures long-horizon chain-of-thought on 2,500 expert-designed problems spanning chemistry, mathematics, computer science, chess, and logic — short inputs, verifiable answers, requiring navigation of a graph of interdependent steps spanning tens to hundreds of thousands of reasoning tokens. At release the best models scored under 10% (GPT-5.2: 9.8%; Gemini 3 Pro: 6.1%). The decisive feature is the construction: **each local step is individually tractable for frontier models**, so failures can only be attributed to long-horizon reasoning itself rather than to any missing single-step capability. This yields a grading constraint: **task horizon (number of required steps and strength of cross-step dependencies) is recorded at L0 as an independent input; the capability level of the model in use may not be invoked to lower the G grade or to waive any tier of oversight intensity.** ⚠️ Limitation: this benchmark measures **single-pass reasoning without tools, external state, or feedback**. It may **not** be read as "even with a harness, long-horizon tasks remain unsolvable" — that is precisely what a harness exists to address. It only refutes the claim that model capability can substitute for attending to task horizon. Evidence grade B.
