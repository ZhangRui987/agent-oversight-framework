...
**文档使用 CC BY-SA 4.0**：复制、翻译、改编保持自由，衍生作品沿用相同协议，防止治理框架被闭源私有化。

**代码使用 Apache-2.0**：为实现者提供明确的专利授权。MIT 对专利保持沉默，这是不选 MIT 的原因。

## 贡献

贡献落入你修改的文件所适用的许可证。提交时使用 `git commit -s` 以证明 Developer Certificate of Origin（DCO）。

## 商标

本许可证仅覆盖著作权，不授予任何名称、标志或品牌使用权。见 [`TRADEMARKS.md`](./TRADEMARKS.md)。

## 历史

v1.0.0 为本仓库首次对外发布。内部演进史（v1 → v2.3.3）见 [`CHANGELOG.md`](./CHANGELOG.md)。
